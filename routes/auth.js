...
router.post("/login/student", async (req, res) => {
  ...
  try {
    if (!mat_no) return res.status(400).send({ message: "Matriculation number is required" });
    if (!password) return res.status(400).send({ message: "Password is required" });

    const result = await pool.query("SELECT mat_no AS id, password FROM students WHERE mat_no = $1", [mat_no]);
...
router.post("/register/student", async (req, res) => {
  let { mat_no, first_name, last_name, department_code, email, password } = req.body;
  ...
  try {
    if (!mat_no) return res.status(400).send({ message: "Matriculation number is required" });
    if (!password) return res.status(400).send({ message: "Password is required" });
    if (!first_name) return res.status(400).send({ message: "First Name is required" });
    if (!last_name) return res.status(400).send({ message: "Last Name is required" });
    if (!department_code) return res.status(400).send({ message: "Department code is required" });
    ...
    await pool.query(
      `INSERT INTO students (mat_no, first_name, last_name, department_code, email, password)
            VALUES ($1, $2, $3, $4, $5, $6)`,
      [mat_no, first_name, last_name, department_code, email, hashedPassword]
    );
    ...
    if (err.code == "23505" && err.constraint == "students_pkey") return res.status(400).send({ message: "User already exists" });
    ...
    if (err.constraint == "students_mat_no_check") return res.status(400).send({ message: "Not a valid matriculation number" });
...
router.post("/register/lecturer", async (req, res) => {
  ...
  try {
    if (!staff_id) return res.status(400).send({ message: "Staff Id is required" });
    if (!password) return res.status(400).send({ message: "Password is required" });
    if (!first_name) return res.status(400).send({ message: "First Name is required" });
    if (!last_name) return res.status(400).send({ message: "Last Name is required" });
    if (!email) return res.status(400).send({ message: "Email is required" });
...
router.post("/login/lecturer", async (req, res) => {
  ...
  try {
    ...
    const result = await pool.query("SELECT staff_id AS id, password FROM lecturers WHERE staff_id = $1", [staff_id]);
    if (result.rows.length === 0) {
