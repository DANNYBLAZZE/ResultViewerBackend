...
router.use((req, res, next) => {
  ...
  if (req.session.user.role != "lecturer") return res.status(403).send({ message: "Not Authorized" });
  next();
});
...
router.get("/get_details", async (req, res) => {
  try {
    ...
    const { rows } = await pool.query("SELECT staff_id as id, first_name, last_name, email FROM lecturers WHERE staff_id = $1", [
      staffId,
    ]);
    res.json(rows[0]); 
  } catch (err) {
...
router.post("/upload-result", upload.single("results"), async (req, res) => {
  if (!req.file) return res.status(400).send({ message: "No data passed in the request body" });
  ...
  try {
    ...
    console.log(error);
    if (error.code == "23503") return res.status(400).send({ message: "Mat No does not exist" });
